...
function getStudentFullName(student) {
  const full = `${student?.nom || ''} ${student?.prenom || ''}`.trim();
  return full || student?.name || 'Étudiant';
}
  ...
  return (
    <div className="layout-grid layout-grid--balanced fade-in scroll-reveal">
      {safeStudents.map((student, index) => {
        ...
        const lineLabel = line?.name || student.busLine || 'Ligne non renseignée';
        const plan = planLookup[student.subscriptionPlan] || { label: student.subscriptionPlan || 'Abonnement' };
        const promoLabel = (student.promo || student.niveau || '').trim();
        const classLabel = (student.classe || student.classGroup || '').trim();

        const expirationDate = student.subscriptionExpiresAt
        ...
        const monthlyFee = typeof student.monthlyFee === 'number'
          ? student.monthlyFee
          : typeof student.montantMensuel === 'number'
            ? student.montantMensuel
            : null;
        const paidMonthsCount = student.subscription?.monthsPaidCount ?? (student.monthsPaid?.length ?? 0);
        const qrToken = student.qrCode?.token || null;

        return (
          <article key={student.id || `${index}`} className="card stagger-item fade-in" style={{ overflow: 'hidden', animationDelay: `${index * 0.05}s` }}>
            <div className={statusClass} />
            <div style={{ padding: '1.3rem' }}>
              <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 700 }}>{getStudentFullName(student)}</h3>
                  <p className="subtitle" style={{ marginTop: '0.2rem' }}>{lineLabel}</p>
                </div>
                <div className="chips">
                  {promoLabel ? <span className="chip chip--primary">{promoLabel}</span> : null}
                  {classLabel ? <span className="chip chip--primary">{classLabel}</span> : null}
                  {statusBadge ? (
                    <span
                      className="badge"
                      style={{
              ...
              <section className="layout-grid" style={{ gap: '0.75rem' }}>
                <div>
                  ...
                  <p style={{ margin: '0.2rem 0', fontWeight: 600 }}>{student.contact || student.email || 'N/A'}</p>
                  ...
                  {!qrToken ? (
                    ...
                    <>
                      <div id={`qr-container-${student.id}`}>
                        <QRCodeCanvas value={encodeQRCodePayload(student)} size={112} />
                      </div>
                      {onDownloadQR && (
                        <button
              ...
              <section style={{ marginTop: '1rem', display: 'grid', gap: '0.5rem' }}>
                <div className="chips">
                  ...
                  {typeof monthlyFee === 'number' && monthlyFee > 0 && (
                    <span className="chip">{monthlyFee.toLocaleString('fr-FR')} FCFA</span>
                  )}
                  <span className="chip">Payé: {paidMonthsCount} mois</span>
...
function encodeQRCodePayload(student) {
  ...
  const payload = {
    ...
    classGroup: student?.classe || student?.classGroup || '',
