...
export default function StudentFormModal({ student, onClose, onSave }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!classes.length) {
      nextErrors.classe = 'Aucune classe disponible. Ajoutez une classe dans Paramètres.';
    } else if (!classe.trim()) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  <option value="">
                      {loadingOptions ? 'Chargement...' : classes.length ? 'Sélectionner une classe' : 'Aucune classe disponible'}
                  </option>
                  {classes.map((item) => (
                    <option key={item.id} value={item.name}>
                      {item.name}
              ...
              {!loadingOptions && classes.length === 0 && (
                <p className="text-xs text-red-600 mt-1">
                  Ajoutez d'abord une classe dans Paramètres &gt; Classes/Promos.
                </p>
              )}
            </div>
