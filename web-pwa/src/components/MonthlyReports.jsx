...
export default function MonthlyReports({ students = [], onClose, open = true, onReSubscribe, lines = [] }) {
  const [sessionId, setSessionId] = useState(() => {
    // Initialiser avec la session actuelle de maniËre asynchrone
    return null; // Sera mis ‡ jour dans useEffect
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        // GÈnÈrer la liste des sessions (mois) disponibles
        ...
        console.warn('Erreur gÈnÈration liste sessions:', err);
        // Fallback: utiliser la date actuelle
        const now = new Date();
      ...
      <div style={modalStyle} className="modal-enter">
        ...
        <div>
          ...
          <div className="toolbar" style={{ marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              <button className="button" type="button" onClick={handleExport}>
                <FileDown size={16} /> TÈlÈcharger CSV
              </button>
              <button className="button" type="button" onClick={handleExportPDF}>
                <FileText size={16} /> TÈlÈcharger PDF
              </button>
          ...
          <div className="card" style={{ padding: '1rem', marginBottom: '1rem' }}>
            <h4 className="section-title" style={{ marginBottom: '0.5rem' }}>RÈsumÈ</h4>
            <div className="chips" style={{ gap: '0.5rem', flexWrap: 'wrap' }}>
              <span className="chip">Total Ètudiants: {summary.totalStudents}</span>
              <span className="chip chip--success">PayÈs dans dÈlai: {summary.paidOnTime}</span>
              <span className="chip" style={{ background: 'rgba(251, 191, 36, 0.2)', borderColor: 'rgba(251, 191, 36, 0.5)' }}>
                PayÈs en gr‚ce: {summary.paidInGrace}
              </span>
              {summary.paidInAdvance > 0 && (
                <span className="chip" style={{ background: 'rgba(34, 197, 94, 0.2)', borderColor: 'rgba(34, 197, 94, 0.5)', fontWeight: 600 }}>
                  PayÈs en avance: {summary.paidInAdvance} ({summary.totalAdvanceMonths || 0} mois)
                </span>
              )}
              <span className="chip">Gr‚ce active (impayÈs fin mois): {summary.unpaid}</span>
              <span className="chip chip--danger">DÈfaillants: {summary.defaulters}</span>
              <span className="chip">PayÈs hors dÈlai: {summary.paidOutOfGrace}</span>
              <span className="chip" style={{ background: '#eef2ff', borderColor: '#c7d2fe', fontWeight: 600 }}>
                Total perÁu pour ce mois: {Number(summary.totalAmountForSession || 0).toLocaleString('fr-FR')} FCFA
              </span>
            </div>
          </div>

          <div className="card" style={{ padding: '1rem' }}>
            <h4 className="section-title" style={{ marginBottom: '0.5rem' }}>DÈtails</h4>
                      ...
                      <td>
                        ...
                        {r.advanceInfo && r.advanceInfo.totalAdvanceMonths > 1 && (
                          <small style={{ display: 'block', fontSize: '0.75rem', color: '#64748b', marginTop: '0.25rem' }}>
                            Total: {r.advanceInfo.totalAdvanceMonths} mois payÈs en avance
                          </small>
                        )}
                      </td>
                      <td>
          ...
          <div className="card" style={{ padding: '1rem', marginTop: '1rem', background: '#f8fafc' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              <h4 className="section-title" style={{ margin: 0 }}>Rapports AvancÈs</h4>
              <button
                className="button button--subtle"
                type="button"
                ...
                style={{ padding: '0.5rem 0.75rem' }}
              >
                {showAdvanced ? 'Masquer' : 'Afficher'} Rapports AvancÈs
              </button>
            </div>
              ...
              <div style={{ display: 'grid', gap: '1rem', marginTop: '1rem' }}>
                ...
                <div className="card" style={{ padding: '1rem', background: 'white' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.75rem' }}>
                    ...
                    >
                      <option value="">SÈlectionner session 1</option>
                    ...
                    >
                      <option value="">SÈlectionner session 2</option>
                      {monthsOptions.map(opt => (
                        <option key={opt.id} value={opt.id}>{opt.label}</option>
                      ))}
                    </select>
                      ...
                      {(() => {
                        ...
                        return (
                          <div style={{ display: 'grid', gap: '0.5rem', fontSize: '0.85rem' }}>
                            <div><strong>DiffÈrence Total Ètudiants:</strong> {comparison.comparison.totalStudentsDiff > 0 ? '+' : ''}{comparison.comparison.totalStudentsDiff}</div>
                            <div><strong>DiffÈrence PayÈs ‡ temps:</strong> {comparison.comparison.paidOnTimeDiff > 0 ? '+' : ''}{comparison.comparison.paidOnTimeDiff}</div>
                            <div><strong>DiffÈrence DÈfaillants:</strong> {comparison.comparison.defaultersDiff > 0 ? '+' : ''}{comparison.comparison.defaultersDiff}</div>
                          </div>
                        );
                      })()}
                    </div>
                ...
                {/* PrÈdictions de revenus */}
                <div className="card" style={{ padding: '1rem', background: 'white' }}>
                  <h5 style={{ marginBottom: '0.75rem', fontSize: '0.95rem', fontWeight: 600 }}>PrÈdictions de Revenus</h5>
                  <div style={{ marginBottom: '0.75rem' }}>
                    <label style={{ display: 'block', marginBottom: '0.25rem', fontSize: '0.85rem' }}>
                      Nombre de mois ‡ prÈdire:
                      <input
                        className="input-field"
                        type="number"
                        min={1}
                    ...
                    return (
                      <div style={{ padding: '0.75rem', background: '#f8fafc', borderRadius: '8px' }}>
                        ...
                        <div style={{ fontSize: '0.85rem', marginBottom: '0.5rem' }}>
                          <strong>…tudiants actifs:</strong> {predictions.currentActiveStudents}
                        </div>
                        <div style={{ marginTop: '0.75rem', fontSize: '0.85rem' }}>
                          <strong>PrÈdictions:</strong>
          ...
          <div className="card" style={{ padding: '1rem', marginTop: '1rem' }}>
            <h4 className="section-title" style={{ marginBottom: '0.5rem' }}>Historique des bilans archivÈs</h4>
            <ul className="list">
              {(history || []).map(item => (
              ...
              {(!history || history.length === 0) && <li className="list__item">Aucun bilan archivÈ pour le moment.</li>}
            </ul>
          </div>
        </div>
      </div>
